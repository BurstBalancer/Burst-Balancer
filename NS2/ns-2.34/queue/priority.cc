...
public:
  ...
  TclObject *create(int, const char *const *) { return (new Priority); }
} class_priority;
...
void Priority::enque(Packet *p) {
  ...
  // fprintf(stderr,
  //         "queueing sn %d rn %d packet src %d dst %d fid %d prio %d len %d ts
  //         "
  //         "% lf\n ",
  //         src_, dst_, iph->saddr(), iph->daddr(), iph->flowid(), prio,
  //         q_[prio]->byteLength(), Scheduler::instance().clock());
  ...
  if ((marking_scheme_ == PER_QUEUE_ECN &&
       q_[prio]->byteLength() > thresh_ * mean_pktsize_) ||
