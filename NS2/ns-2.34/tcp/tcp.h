...
#define CWND_ACTION_EXITED                                                     \
  4 // congestion recovery has ended
...
struct hstcp {
  ...
  hstcp()
      : low_p(0.0), dec1(0.0), dec2(0.0), p1(0.0), p2(0.0), cwnd_last_(0.0),
        increase_last_(0.0) {}
};
...
#define T_RTT_BITS 0
  ...
  virtual void cancel_rtx_timer() { rtx_timer_.force_cancel(); }
...
protected:
  double vegastime() { return (Scheduler::instance().clock() - firstsent_); }
  virtual void output(int seqno, int reason = 0);
  virtual void recv_newack_helper(Packet *);
