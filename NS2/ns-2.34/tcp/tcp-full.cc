...
static const char rcsid[] =
    "@(#) $Header: /cvsroot/nsnam/ns-2/tcp/tcp-full.cc,v 1.128 2009/03/29 "
    "20:59:41 sallyfloyd Exp $ (LBL)";
#endif
...
public:
  ...
  TclObject *create(int, const char *const *) { return (new FullTcpAgent()); }
} class_full;
...
public:
  ...
  TclObject *create(int, const char *const *) { return (new MinTcpAgent()); }
} class_min_full;

static class DDTcpClass : public TclClass {
public:
  DDTcpClass() : TclClass("Agent/TCP/FullTcp/Sack/DDTCP") {}
  TclObject *create(int, const char *const *) { return (new DDTcpAgent()); }
} class_dd_full;
...
int FullTcpAgent::delay_bind_dispatch(const char *varName,
  ...
  if (delay_bind(varName, localName, "segsize_", &maxseg_, tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "tcprexmtthresh_", &tcprexmtthresh_,
                 tracer))
    return TCL_OK;
  ...
  if (delay_bind_bool(varName, localName, "nodelay_", &nodelay_, tracer))
    return TCL_OK;
  if (delay_bind_bool(varName, localName, "data_on_syn_", &data_on_syn_,
                      tracer))
    return TCL_OK;
  if (delay_bind_bool(varName, localName, "dupseg_fix_", &dupseg_fix_, tracer))
    return TCL_OK;
  if (delay_bind_bool(varName, localName, "dupack_reset_", &dupack_reset_,
                      tracer))
    return TCL_OK;
  if (delay_bind_bool(varName, localName, "close_on_empty_", &close_on_empty_,
                      tracer))
    return TCL_OK;
  if (delay_bind_bool(varName, localName, "signal_on_empty_", &signal_on_empty_,
                      tracer))
    return TCL_OK;
  if (delay_bind_time(varName, localName, "interval_", &delack_interval_,
                      tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "ts_option_size_", &ts_option_size_,
                 tracer))
    return TCL_OK;
  if (delay_bind_bool(varName, localName, "reno_fastrecov_", &reno_fastrecov_,
                      tracer))
    return TCL_OK;
  if (delay_bind_bool(varName, localName, "pipectrl_", &pipectrl_, tracer))
    return TCL_OK;
  if (delay_bind_bool(varName, localName, "open_cwnd_on_pack_",
                      &open_cwnd_on_pack_, tracer))
    return TCL_OK;
  ...
  if (delay_bind_bool(varName, localName, "debug_", &debug_, tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "flow_remaining_", &flow_remaining_,
                 tracer))
    return TCL_OK; // Mohammad
  if (delay_bind(varName, localName, "dynamic_dupack_", &dynamic_dupack_,
                 tracer))
    return TCL_OK; // Mohammad
  ...
  if (delay_bind(varName, localName, "deadline", &deadline, tracer))
    return TCL_OK; // Shuang
  if (delay_bind(varName, localName, "early_terminated_", &early_terminated_,
                 tracer))
    return TCL_OK; // Shuang

  if (delay_bind_bool(varName, localName, "enable_pias_", &enable_pias_,
                      tracer))
    return TCL_OK; // wei
  if (delay_bind(varName, localName, "pias_prio_num_", &pias_prio_num_, tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "pias_thresh_0", &pias_thresh_[0], tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "pias_thresh_1", &pias_thresh_[1], tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "pias_thresh_2", &pias_thresh_[2], tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "pias_thresh_3", &pias_thresh_[3], tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "pias_thresh_4", &pias_thresh_[4], tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "pias_thresh_5", &pias_thresh_[5], tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "pias_thresh_6", &pias_thresh_[6], tracer))
    return TCL_OK;
  if (delay_bind_bool(varName, localName, "pias_debug_", &pias_debug_, tracer))
    return TCL_OK;
...
int SackFullTcpAgent::delay_bind_dispatch(const char *varName,
                                          const char *localName,
                                          TclObject *tracer) {
  if (delay_bind_bool(varName, localName, "clear_on_timeout_",
                      &clear_on_timeout_, tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "sack_rtx_cthresh_", &sack_rtx_cthresh_,
                 tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "sack_rtx_bthresh_", &sack_rtx_bthresh_,
                 tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "sack_rtx_threshmode_",
                 &sack_rtx_threshmode_, tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "sack_block_size_", &sack_block_size_,
                 tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "sack_option_size_", &sack_option_size_,
                 tracer))
    return TCL_OK;
  if (delay_bind(varName, localName, "max_sack_blocks_", &max_sack_blocks_,
                 tracer))
    return TCL_OK;
...
void FullTcpAgent::advance_bytes(int nb) {
  ...
  case TCPS_CLOSED:
    // if (here_.addr_ == 3 && dst_.addr_ == 7 && fid_ == 108) {
    //   fprintf(stderr,
    //           "FullTcpAgent::advance_bytes(nb: %d, stats_: TCPS_CLOSED)\n",
    //           nb);
    // }
  case TCPS_LISTEN:
    // if (here_.addr_ == 3 && dst_.addr_ == 7 && fid_ == 108) {
    //   fprintf(stderr,
    //           "FullTcpAgent::advance_bytes(nb: %d, stats_: TCPS_LISTEN)\n",
    //           nb);
    // }
    reset();
    startseq_ = iss_;
  ...
  case TCPS_ESTABLISHED:
    // if (here_.addr_ == 3 && dst_.addr_ == 7 && fid_ == 108) {
    //   fprintf(stderr,
    //           "FullTcpAgent::advance_bytes(nb: %d, stats_:
    //           TCPS_ESTABLISHED)\n", nb);
    // }
  case TCPS_SYN_SENT:
    // if (here_.addr_ == 3 && dst_.addr_ == 7 && fid_ == 108) {
  ...
  case TCPS_SYN_RECEIVED:
    ...
    //   fprintf(
    //       stderr,
    //       "FullTcpAgent::advance_bytes(nb: %d, stats_: TCPS_SYN_RECEIVED)\n",
    //       nb);
    // }
    if (curseq_ < iss_)
      curseq_ = iss_;
...
void FullTcpAgent::bufferempty() {
  ...
  // if (here_.addr_ == 3 && dst_.addr_ == 7 && fid_ == 108) {
  //   fprintf(stderr, "FullTcpAgent::bufferempty(%s)\n", this->name());
  // }
  ...
  case TCPS_CLOSING:
    ...
    if (debug_)
      fprintf(stderr, "%f FullTcpAgent(%s): app close in bad state %s\n", now(),
              name(), statestr(state_));
    break;
  default:
    if (debug_)
      fprintf(stderr, "%f FullTcpAgent(%s): app close in unknown state %s\n",
              now(), name(), statestr(state_));
  }
...
void FullTcpAgent::prpkt(Packet *pkt) {
  ...
  fprintf(stdout,
          " [%d:%d.%d>%d.%d] (hlen:%d, dlen:%d, seq:%d, ack:%d, flags:0x%x "
          "(%s), salen:%d, reason:0x%x)\n",
          th->uid(), iph->saddr(), iph->sport(), iph->daddr(), iph->dport(),
          tcph->hlen(), datalen, tcph->seqno(), tcph->ackno(), tcph->flags(),
          flagstr(tcph->flags()), tcph->sa_length(), tcph->reason());
}

char *FullTcpAgent::flagstr(int hflags) {
  ...
  static char *flagstrs[28] = {
      ...
      "<PSH,ACK>", "<PSH,ACK,FIN>", "<PSH,ACK,SYN>",
      "<PSH,ACK,SYN,FIN>", // 0x18-0x1b
  };
...
char *FullTcpAgent::statestr(int state) {
...
void DelAckTimer::expire(Event *) { a_->timeout(TCP_TIMER_DELACK); }
...
void FullTcpAgent::finish() { Tcl::instance().evalf("%s done", this->name()); }
/*
 * headersize:
...
int FullTcpAgent::reass(Packet *pkt) {
  ...
  if (start == end && (tiflags & TH_FIN) == 0) {
    fprintf(stderr,
            "%f: FullTcpAgent(%s)::reass() -- bad condition - adding non-FIN "
            "zero-len seg\n",
            now(), name());
    abort();
  }
...
int FullTcpAgent::rcvseqinit(int seq, int dlen) {
  return (seq + dlen + 1);
  // printf("newww3 fid= %d, rcv_nxt_= %d diff= %d, highest_ack= %d,
  // last_ack_sent= %d diff=
...
void FullTcpAgent::ack_action(Packet *p) { FullTcpAgent::pack_action(p); }
...
int FullTcpAgent::calPrio(int prio) {
  ...
  for (int i = 1; i < prio_num_; i++)
    if (prio <= prio_cap_[i * 8 / prio_num_ - 1]) {
      // printf("prio %d cap %d ans %d\n", prio, prio_cap_[i*8/prio_num_ - 1], i
      // - 1);
      return i - 1;
    }

  // printf("prio %d cap %d ans %d\n", prio, prio_cap_[8/prio_num_ - 1],
  // prio_num_ - 1);
  return prio_num_ - 1;
}
...
int FullTcpAgent::piasPrio(int bytes_sent) {
  if (enable_pias_ && pias_prio_num_ >= 1) {
    ...
    for (int i = 0; i < pias_prio_num_ - 1;
         i++) // there are pias_prio_num-1 demotion thresholds in total
    {
 ...
 */
void FullTcpAgent::sendpacket(int seqno, int ackno, int pflags, int datalen,
                              int reason, Packet *p) {
  if (!p)
    p = allocpkt();
  ...
  // fprintf(stderr,
  //         "FullTcpAgent::sendpacket(seqno: %d, ackno: %d, curseq_: %d)\n",
  //         seqno, ackno, int(curseq_));
  ...
  if (datalen > 0) {
    ...
    if (deadline == 0) {
      ...
      if (tleft < 0 || byterm() * 8 / 1e4 > tleft) {
        ...
        //				iph->prio() = iph->prio() / 40 * 1000 +
        // set_prio(seqno, curseq_) / 1460;
      }
    }
...
int FullTcpAgent::foutput(int seqno, int reason) {
  ...
  int buffered_bytes =
      (infinite_send_) ? TCP_MAXSEQ : curseq_ - highest_ack_ + 1;
  ...
  //		printf("%.5lf: FLOW%d: win %d probe: %d buffered bytes %d off %d
  // seqno %d, highestack %d, datalen %d\n", now(), fid_, win, tmp,
  // buffered_bytes, off, seqno, tmph, datalen); 		fflush(stdout);
  //	}

  //	if (deadline != 0 && !syn) {
  //		double tleft = deadline/1e6 - (now() - start_time);
  //		if (tleft < 0) {
  //			printf("early termination now %.8lf start %.8lf deadline
  //%d\n", now(), start_time, deadline); 			fflush(stdout);
  // buffered_bytes = 0; 			datalen = 0;
  //		}
  //    }
  // if (here_.addr_ == 3 && dst_.addr_ == 7 && fid_ == 108) {
  //   fprintf(stderr,
  //           "FullTcpAgent::foutput(seqno: %d, signal_on_empty_: %d, "
  //           "buffered_bytes: %d, syn: %d)\n",
  //           seqno, signal_on_empty_, buffered_bytes, syn);
  // }
  ...
  // printf("%f %d %d\n", Scheduler::instance().clock(), (int) highest_ack_,
  // (int) maxseq_);
  ...
  if (!infinite_send_ &&
      ((seqno + datalen) > curseq_ || (syn && datalen == 0))) {
  ...
  if (need_send()) {
    //		if(fid_==2352) printf("before need_send fid= %d, rcv_nxt_= %d
    // highest_ack= %d, last_ack_sent=
  ...
  if ((flags_ & (TF_ACKNOW | TF_NEEDCLOSE)) || (pflags & (TH_SYN | TH_FIN))) {
    goto send;
  }
...
void FullTcpAgent::send_much(int force, int reason, int maxburst) {
  ...
  // if ((int(t_seqno_)) > 1)
  // printf("%f: send_much(f:%d, win:%d, pipectrl:%d, pipe:%d, t_seqno:%d,
  // topwin:%d, maxseq_:%d\n", now(), force, win, pipectrl_, pipe_,
  // int(t_seqno_), topwin, int(maxseq_));

  // if (fid_ == 658) {
  //   fprintf(stderr, "FullTcpAgent::send_much(breakpoint: 1)\n");
  // }
  ...
  while (1) {
    ...
    // if (fid_ == 658) {
    //   fprintf(stderr,
    //           "FullTcpAgent::send_much(breakpoint: 2, seqno: %d, ts: %lf)\n",
    //           seq, Scheduler::instance().clock());
    // }
    if (!force && !send_allowed(seq))
      break;
    // Q: does this need to be here too?
    // if (fid_ == 658) {
    //   fprintf(stderr, "FullTcpAgent::send_much(breakpoint: 3)\n");
    // }
    ...
    // if (fid_ == 658) {
    //   fprintf(stderr, "FullTcpAgent::send_much(breakpoint: 4)\n");
    // }
...
void FullTcpAgent::newack(Packet *pkt) {
  ...
  // last_sqtotal_,int(ackno), int(highest_ack_));
  if (ackno == maxseq_) {
  ...
  if (!fh->no_ts_) {
    if (ts_option_) {
      ...
      if (ts_resetRTO_ &&
          (!ect_ || !ecn_backoff_ || !hdr_flags::access(pkt)->ecnecho())) {
        // From Andrei Gurtov
        //
...
int FullTcpAgent::idle_restart() {
  ...
  if (!ts_option_) {
    double tickoff = fmod(last_send_time_ + boot_time_, tcp_tick_);
...
void FullTcpAgent::recv(Packet *pkt, Handler *) {
  ...
  //   hdr_ip *iph = hdr_ip::access(pkt);
  //   if (iph->flowid() == 658) {
  //     fprintf(stderr, "FullTcpAgent::recv(seqno: %d, fid: %d)\n",
  //     tcph->seqno(),
  //             fid_);
  //   }
  // }
  ...
  // fprintf(stdout, "%f(%s)in state %s recv'd this packet: ", now(), name(),
  // statestr(state_)); prpkt(pkt);
  //}
  /*
  ...
  if (state_ == TCPS_CLOSED) {
    if (debug_) {
      fprintf(stderr, "%f: FullTcp(%s): recv'd pkt in CLOSED state: ", now(),
              name());
      prpkt(pkt);
    }
    goto drop;
  ...
  /*if (datalen > 0)
    printf("received data: datalen = %d seqno = %d, ackno = %d, ce = %d,
  ecn-echo = %d\n", datalen, tcph->seqno(), ackno, fh->ce(), fh->ecnecho());
  else
    printf("received ack : datalen = %d seqno = %d, ackno = %d, ce = %d,
  ecn-echo = %d\n", datalen, tcph->seqno(), ackno, fh->ce(), fh->ecnecho());
  */
  ...
  if (!nopredict_ && predict_ok(pkt)) {
    ...
    if (datalen == 0) {
      ...
      if (ackno > highest_ack_ && ackno < maxseq_ && cwnd_ >= wnd_ &&
          !fastrecov_) {
      ...
      if (need_send()) {
        send_much(1, REASON_NORMAL, maxburst_);
        //				if(fid_==2352) printf("before2 need_send
        // fid= %d, rcv_nxt_= %d highest_ack= %d, last_ack_sent=
        //%d\n",fid_,(int)rcv_nxt_,(int)highest_ack_,last_ack_sent_);
      }
      return;
    }
  ...
  case TCPS_SYN_SENT: /* we sent SYN, expecting SYN+ACK (or SYN) */
    ...
    if ((tiflags & TH_ACK) && ((ackno <= iss_) || (ackno > maxseq_))) {
      // not an ACK for our SYN, discard
      if (debug_) {
        fprintf(stderr,
                "%f: FullTcpAgent::recv(%s): bad ACK for our SYN: ", now(),
                name());
        prpkt(pkt);
      }
      goto dropwithreset;
    ...
    if ((tiflags & TH_SYN) == 0) {
      if (debug_) {
        fprintf(stderr,
                "%f: FullTcpAgent::recv(%s): no SYN for our SYN: ", now(),
                name());
        prpkt(pkt);
      }
      goto drop;
  ...
  if ((tiflags & TH_ACK) == 0) {
    ...
    if (state_ != TCPS_SYN_RECEIVED) {
      if (debug_) {
        fprintf(
            stderr,
            "%f: FullTcpAgent::recv(%s) got packet lacking ACK (state:%d): ",
            now(), name(), state_);
        prpkt(pkt);
      }
    }
  ...
  case TCPS_SYN_RECEIVED: /* want ACK for our SYN+ACK */
    if (ackno < highest_ack_ || ackno > maxseq_) {
      ...
      if (debug_) {
        fprintf(stderr,
                "%f: FullTcpAgent(%s): ack(%d) not in range while in "
                "SYN_RECEIVED: ",
                now(), name(), ackno);
        prpkt(pkt);
      }
  ...
  case TCPS_LAST_ACK:
    ...
    if (fh->ecnecho() && (!ecn_ || !ect_)) {
      fprintf(stderr,
              "%f: FullTcp(%s): warning, recvd ecnecho but I am not ECN "
              "capable! %d %d\n",
              now(), name(), ecn_);
    }
    ...
    if (ackno <= highest_ack_) {
      ...
      if (datalen == 0 && (!dupseg_fix_ || !dupseg)) {
        ...
        if ((rtx_timer_.status() != TIMER_PENDING) || ackno < highest_ack_) {
  ...
  process_ACK:
    ...
    if (ackno > maxseq_) {
      ...
      if (debug_) {
        fprintf(stderr,
                "%f: FullTcpAgent::recv(%s) too-big ACK (maxseq:%d): ", now(),
                name(), int(maxseq_));
        prpkt(pkt);
      }
      goto dropafterack;
     ...
     */
    if ((!delay_growth_ || (rcv_nxt_ > 0)) && last_state_ == TCPS_ESTABLISHED) {
    ...
    case TCPS_LAST_ACK: /* passive close */
      ...
      if (ourfinisacked) {
        ...
        if (debug_) {
          fprintf(stderr,
                  "%f: FullTcpAgent(%s)::received non-ACK (state:%d): ", now(),
                  name(), state_);
          prpkt(pkt);
        }
      }
...
step6:
  ...
  if ((datalen > 0 || (tiflags & TH_FIN)) && TCPS_HAVERCVDFIN(state_) == 0) {
...
void FullTcpAgent::dupack_action() {
  ...
  if (recovered || (!bug_fix_ && !ecn_) ||
      (last_cwnd_action_ == CWND_ACTION_DUPACK) || (highest_ack_ == 0)) {
...
void FullTcpAgent::timeout_action() {
  ...
  // if (fid_ == 658) {
  //   fprintf(stderr, "FullTcpAgent::timeout_action(t_seqno_: %d)\n",
  //           int(t_seqno_));
  // }
...
void FullTcpAgent::timeout(int tno) {
  ...
  if (state_ == TCPS_LISTEN) {
    ...
    if (debug_) {
      fprintf(stderr,
              "%f: FullTcpAgent(%s): unexpected timeout %d in state %s\n",
  ...
  // if (fid_) {
  //   fprintf(stderr, "FullTcpAgent::timeout(tno: %d)\n", tno);
  // }
...
void FullTcpAgent::dooptions(Packet *pkt) {
  ...
  if (ts_option_ && !fh->no_ts_) {
    if (tcph->ts() < 0.0) {
      fprintf(stderr,
              "%f: FullTcpAgent(%s) warning: ts_option enabled in this TCP, "
              "but appears to be disabled in peer\n",
              now(), name());
...
void FullTcpAgent::process_sack(hdr_tcp *) {
  fprintf(
      stderr,
      "%f: FullTcpAgent(%s) Non-SACK capable FullTcpAgent received a SACK\n",
      now(), name());
  return;
}
...
int SackFullTcpAgent::set_prio(int seq, int maxseq) {
  ...
  if (prio_scheme_ == 2) {
    ...
    // printf("%d %d %d %d\n", maxseq, int(highest_ack_), sq_.total(), maxseq -
    // int(highest_ack_) - sq_.total() + 10); fflush(stdout);
...
void SackFullTcpAgent::dupack_action() {
  ...
  // printf("%f: SACK DUPACK-ACTION:pipe_:%d, sq-total:%d, bugfix:%d, cwnd:%d,
  // highest_ack:%d, recover_:%d\n", now(), pipe_, sq_.total(), bug_fix_,
  // int(cwnd_), int(highest_ack_), recover_);
  ...
  if (bug_fix_) {
    ...
    // printf("%f: SACK DUPACK-ACTION BUGFIX RETURN:pipe_:%d, sq-total:%d,
...
full_sack_action:
  ...
  // printf("%f: FAST-RTX seq:%d, h_seqno_ is now:%d, pipe:%d, cwnd:%d,
  // recover:%d\n", now(), int(highest_ack_), h_seqno_, pipe_, int(cwnd_),
  // recover_);
...
void SackFullTcpAgent::process_sack(hdr_tcp *tcph) {
  ...
  if (max_sack_blocks_ <= 0) {
    fprintf(stderr,
            "%f: FullTcpAgent(%s) warning: received SACK block but I am not "
            "SACK enabled\n",
            now(), name());
    return;
  }
...
int SackFullTcpAgent::send_allowed(int seq) {
  ...
  //	printf("%f: PIPECTRL: SEND(%d) AWIN:%d, pipe:%d, cwnd:%d highest_ack:%d
  // sqtotal:%d\n", now(), seq, topawin, pipe_, int(cwnd_), int(highest_ack_),
  // sq_.total());
...
int SackFullTcpAgent::nxt_tseq() {
  ...
  // if (fid_ == 658) {
  //   fprintf(stderr,
  //           "SackFullTcpAgent::nxt_tseq(highest_ack_: %d, recover_: %d)\n",
  //           int(highest_ack_), recover_);
  // }
  ...
  while ((seq = sq_.nexthole(seq, fcnt, fbytes)) > 0) {
    ...
    if (sack_rtx_threshmode_ == 0 ||
        ...
        (sack_rtx_threshmode_ == 3 &&
         (fcnt >= sack_rtx_cthresh_ || fbytes >= sack_rtx_bthresh_)) ||
        (sack_rtx_threshmode_ == 4 &&
         (fcnt >= sack_rtx_cthresh_ && fbytes >= sack_rtx_bthresh_))) {
      ...
      if (seq > h_seqno_)
        h_seqno_ = seq;
      // if (fid_ == 658) {
      //   fprintf(stderr, "SackFullTcpAgent::nxt_tseq(return seq: %d)\n",
      //           int(seq));
      // }
      return (seq);
    } else if (fcnt <= 0)
...
double MinTcpAgent::rtt_timeout() { return minrto_; }
...
void DDTcpAgent::slowdown(int how) {
  ...
  if (deadline != 0) {
    ...
    // start_time, deadline); 	fflush(stdout); 	if (signal_on_empty_);
    //		bufferempty();
    //		return;
    //} else
    if (tleft < 0) {
  ...
  if (precision_reduce_) {
    ...
    if (wnd_option_ == 6) {
      /* binomial controls */
      decreasewin =
          windowd() - (1.0 - decrease_num_) * pow(windowd(), l_parameter_);
    ...
    if (wnd_option_ == 6) {
      /* binomial controls */
      temp =
          (int)(window() - (1.0 - decrease_num_) * pow(window(), l_parameter_));
    } else if ((wnd_option_ == 8) && (cwnd_ > low_window_)) {
      /* experimental highspeed TCP */
      decrease = decrease_param();
      // if (decrease < 0.1)
  ...
  if (how & (CLOSE_CWND_HALF | CLOSE_CWND_RESTART | CLOSE_CWND_INIT |
             CLOSE_CWND_ONE | CLOSE_CWND_ECNHAT))
    cong_action_ = TRUE;
...
int DDTcpAgent::byterm() { return curseq_ - int(highest_ack_) - sq_.total(); }
...
int DDTcpAgent::foutput(int seqno, int reason) {
  if (deadline != 0) {
    ...
    // byterm()*8/1e10;
    double tleft = deadline / 1e6 - (now() - start_time) -
                   (curseq_ - int(maxseq_)) * 8 / 1e10;
    if (tleft < 0 && signal_on_empty_) {
      early_terminated_ = 1;
      bufferempty();
      printf("early termination V2 now %.8lf start %.8lf deadline %d byterm %d "
             "tleft %.8f\n",
