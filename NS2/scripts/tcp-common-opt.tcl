...
TCP_pair instproc start { nr_bytes } {
    ...
    $tcps advance-bytes $nr_bytes
		...
		$self set sent_bytes [expr $sent_bytes + $nr_bytes]
}
...
#according to the flow arrival process.
 ...
 #   $self instvar fid
    ...
    # set tmp_ [expr $tmp_ * 1460]
		...
		set sent_bytes [$apair($pid) set sent_bytes]
		set send_limit [expr 2000000000 - $sent_bytes]
		...
		if { $tmp_ > $send_limit } {
			...
			$apair($nr_pairs) setfid $next_fid  ;# Mohammad: assign next fid
			incr next_fid
    ...
    set fid [$apair($pid) set id] 
    puts "new connect $fid @ pair $pid @ $t"
...
#
    ...
    if { [info exists logfile] } {
		...
		puts $logfile "$tmp_pkts $fldur $rttimes $group_id $fin_fid"
		flush stdout
		flush $logfile
    }
