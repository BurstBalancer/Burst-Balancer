...
void test_bsketch(int sketch_size, double flowlet_thld, double clear_thld, ofstream &outFile){
    ...
    int cnt = 0;
    for(auto it = flowlet_size.begin(); it != flowlet_size.end(); ++it){
...
void test_all(double flowlet_delta, double flow_timeout){
    ...
    string filename = "../results/data_bsketch_bucketsize" + to_string(CELL_PER_BUCKET) + ".csv";
    ...
    filename = "../results/data_cbsketch_bucketsize" + to_string(CELL_PER_BUCKET) + ".csv";
    outFile.open(filename.c_str(), ios::out);
    outFile << "memory(KB)" << "," << "AAE" << "," << "ARE" << "," << "RR" << "," << "PR" << "," << "insertionMops" << "," << "queryMops" << endl;
    for(int i = 10; i <= 100; i += 10)
    ...
    filename = "../results/data_strawman_bucketsize" + to_string(CELL_PER_BUCKET) + ".csv";
    outFile.open(filename.c_str(), ios::out);
    outFile << "memory(KB)" << "," << "AAE" << "," << "ARE" << "," << "RR" << "," << "PR" << "," << "insertionMops" << "," << "queryMops" << endl;
    for(int i = 10; i <= 100; i += 10)
        test_strawman(i, flowlet_delta, flow_timeout, 70, outFile);
    outFile.close();
}
...
int main(){
    ...
    getTopKFlowLet(pkts, flowlet_size, 0.09, 0.45, 200);
    std::cout << "get done." << '\n';
    test_all(0.09, 0.45);
    return 0;
}
